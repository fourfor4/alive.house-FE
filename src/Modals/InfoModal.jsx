...
import { setInfoMessage, useErrors } from "../hooks/useErrors";
...
export default function InfoModal() {
  ...
  useEffect(() => {
    if (!errorState.infoTitle.value) {
      dismiss();
      return;
    }
    setOpen(errorState.infoTitle.value !== "hidden");
    setErrMesg(errorState.infoMsg.value);
    setErrTitle(errorState.infoTitle.value || "");
  }, [errorState.infoTimeStamp?.value]);

  // useEffect(() => {
  //   if (errorState.errorTimeStamp?.value >= errorState.infoTimeStamp.value)
  //     setOpen(false);
  ...
  //   }
  //   setOpen(true);
  // }, [errorState.errorMsg?.value, errorState.errorTitle?.value]);
  const dismiss = () => {
    setOpen(false);
              ...
              <Dialog.Panel className="relative bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-2xl sm:w-full sm:h-full sm:p-6">
                <div className="sm:flex sm:items-start">
                  <div className="mt-5 mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full sm:mx-0 sm:h-10 sm:w-10">
                    <CheckCircleIcon
                      className="h-10 w-10 text-gray -translate-y-6"
                      aria-hidden="true"
                    />
                  </div>
